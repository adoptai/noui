...
def _summarize():
    ...
    src = _SRC.read_text()
    m = re.search(r"def _summarize\(report: dict, report_path\) -> str:.*?(?=\ndef )", src, re.S)
...
_REPORT = {
    ...
    "operations": [
        {"name": "read_credit_card", "goal_reached": False,
         "steps": [{"status": "ok"}, {"status": "error"}, {"status": "ok"}]},
        {"name": "download_last_statement", "goal_reached": True,
...
def test_summary_carries_the_per_operation_outcome():
    ...
    assert "first failure at step 1" in out
...
def test_full_json_is_still_reachable_and_still_the_fallback():
    ...
    src = _SRC.read_text()
    assert '"--json"' in src
    assert "if args.json or not wrote_report:" in src, (
        "the full report must still print when the file could not be written"
    )
