...
def test_live_on_first_try_surfaces_link_without_refresh():
    with patch.object(recording, "resolve_agent_token", return_value="agent"), \
         patch.object(recording, "start", return_value=_session("s1")) as start, \
         patch.object(recording.tabby_client, "create_short_link", return_value="https://t/s/aaa") as sl:
...
def test_stale_session_is_restarted_in_place():
    ...
    with patch.object(recording, "resolve_agent_token", return_value="agent"), \
         patch.object(recording, "start", return_value=_session("s1")) as start, \
         patch.object(recording.tabby_client, "create_short_link",
                      side_effect=[RuntimeError("Cannot open stream"), "https://t/s/live"]), \
         patch.object(recording.tabby_client, "restart_recording_session", return_value=True) as restart:
        out = recording.provision_live_link("workflow", "https://x")
